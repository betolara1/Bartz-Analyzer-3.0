...
import { Boxes, ChevronDown, Edit2, AlertTriangle, Search, FileText } from "lucide-react";
...
export function ItemsSection({ isOpen, onToggle, data }: ItemsSectionProps) {
          ...
          {allItems.length > 0 && (
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full bg-[#111] border border-[#232323] text-white pl-9 pr-3 py-2 rounded-lg text-xs outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500/20 transition-all font-medium"
              />
            </div>
          )}
                    ...
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors group/inner">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/20 active:scale-[0.97] transition-all"
                        >
